...
write_area::write_area(std::vector<corner> corners) {
    ...
    for (int i = 0; i < 2; i++) {
        for (size_t j = 0; j < 4; j++) {
            ...
            if (offset < 0) {
                auto idx_tmp =
                    std::find(corner_names.begin(), corner_names.end(), ordered_corners[j].code.data.substr(0, 2));
...
bool write_area::is_valid() {
    if (!this->TL.code.data.empty() && TL.code.data.substr(0, 2) == "TL" && TL.code.data.length() > 3) {
        return true;
    } else if (!this->TR.code.data.empty() && TR.code.data.substr(0, 2) == "TR" && TR.code.data.length() > 3) {
        return true;
    } else if (!this->BR.code.data.empty() && BR.code.data.substr(0, 2) == "BR" && BR.code.data.length() > 3) {
        return true;
    } else if (!this->BL.code.data.empty() && BL.code.data.substr(0, 2) == "BL" && BL.code.data.length() > 3) {
        return true;
    } else {
        return false;
    }
...
[[nodiscard]] cv::Mat write_area::centerize_image(cv::Mat& src) {
    ...
    std::array<cv::Point2f, 4> src_vertice{TL.point, TR.point, BR.point, BL.point};
    std::array<cv::Point2f, 4> dest_vertice{cv::Point(0, 0), cv::Point(dest_width, 0),
                                            cv::Point(dest_width, dest_height), cv::Point(0, dest_height)};

...
namespace handfont {
px constexpr mm_to_px(mm input_mm) { return (input_mm * dpi) / mm_per_inch; }
void error_handler(HPDF_STATUS error_no, HPDF_STATUS detail_no, void* user_data) {
    ...
    std::string err_msg =
        "ERROR: error_no=0x" + to_hex((unsigned int)error_no) + ", detail_no=0x" + to_hex((int)detail_no);
    err_msg += "\n\nBacktrace:";
...
mm input_pdf_generator::draw_write_grid(grid_size size, char_width width, guide_type type, bool is_Fixed_Based, px BL_x,
                                        px BL_y) {
    px grid_height;
    mm result_height;
    px grid_width;
...
void input_pdf_generator::draw_info_grid(grid_size size, char_width width, px BL_x, px BL_y, Unicode character,
    ...
    auto char_width =
        HPDF_Font_GetUnicodeWidth(using_fonts[current_using_fontname], character) * (mm_to_px(font_size) / 1000.0);
    ...
    HPDF_Page_TextOut(current_page, BL_x, BL_y + grid_height - mm_to_px(font_size_alter),
                      ("U+" + to_hex((int)character)).c_str());
    HPDF_Page_EndText(current_page);
    HPDF_Page_GRestore(current_page);
    /*end alternative unicode text*/
}

void input_pdf_generator::draw_grid_set(grid_size size, character_info current_char, bool is_Fixed_Based, px BL_x,
                                        px BL_y) {
...
void input_pdf_generator::add_page(chardef_filemeta filemeta) {
    ...
    HPDF_Page_DrawImage(current_page, qr_codes["TR"], page_width - qr_offset - qr_size,
                        page_height - qr_offset - qr_size, qr_size, qr_size);
    HPDF_Page_DrawImage(current_page, qr_codes["TL"], qr_offset, page_height - qr_offset - qr_size_TL, qr_size_TL,
                        qr_size_TL);
    ...
    HPDF_Page_TextOut(current_page, (page_width - text_width) / 2.0,
                      page_height - mm_to_px(5.0) - mm_to_px(distance_line), font_name.c_str());
    text_width = HPDF_Page_TextWidth(current_page, to_string(data.f_type).c_str());
    HPDF_Page_TextOut(current_page, (page_width - text_width) / 2.0,
                      page_height - mm_to_px(5.0) - mm_to_px(2 * distance_line), to_string(data.f_type).c_str());
    text_width = HPDF_Page_TextWidth(current_page, char_typenames.c_str());
    HPDF_Page_TextOut(current_page, (page_width - text_width) / 2.0,
                      page_height - mm_to_px(5.0) - mm_to_px(3 * distance_line), char_typenames.c_str());
    HPDF_Page_EndText(current_page);
    ...
    draw_corner_marker(mm_to_px(width_grids_area) + garea_x_offset, mm_to_px(height_grids_area) + garea_y_offset,
                       corner_rotation::TR);
    ...
    for (character_info current_char : data.char_infos) {
        if (current_char.width == char_width::FULL &&
            ((data.size == grid_size::SMALL &&
              current_internal_id % num_internal_cols::SMALL == num_internal_cols::SMALL - 1) ||
             (data.size == grid_size::LARGE &&
              current_internal_id % num_internal_cols::LARGE == num_internal_cols::LARGE - 1))) {
            current_internal_id++;
        }
        switch (data.size) {
            case grid_size::SMALL:
                ...
                current_BL_y =
                    height_grids_area - ((floor((double)current_internal_id / num_internal_cols::SMALL) + 1) *
                                         (height_current_grid + height_grid::SMALL));
            ...
            case grid_size::LARGE:
                ...
                current_BL_y =
                    height_grids_area - ((floor((double)current_internal_id / num_internal_cols::LARGE) + 1) *
                                         (height_current_grid + height_grid::SMALL));
                current_BL_x = (current_internal_id % num_internal_cols::LARGE) * (height_grid::LARGE / 2.0);
                break;
        }
        draw_grid_set(data.size, current_char, data.is_Fixed_Base, garea_x_offset + mm_to_px(current_BL_x),
                      garea_y_offset + mm_to_px(current_BL_y));
...
void input_pdf_generator::saveto_file(const std::string filename) { HPDF_SaveToFile(pdf, filename.c_str()); }

input_pdf_generator::~input_pdf_generator() { HPDF_Free(pdf); }
}  // namespace handfont
